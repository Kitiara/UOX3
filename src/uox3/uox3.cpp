    ...
    while (!messageLoop.Empty())
    {
        ...
        switch(tVal.actualMessage)
        {
            ...
            case MSG_COUNT:
                break; 
            case MSG_WORLDSAVE:
                cwmWorldState->SetOldTime(0);
                break;
            case MSG_PRINT:
                Console << tVal.data << myendl;
                break;
            case MSG_RELOADJS:
                JSEngine->Reload();
                JSMapping->Reload();
            ...
            case MSG_RELOAD:        
                if (!cwmWorldState->GetReloadingScripts())
                {
                    ...
                    switch(tVal.data[0])
                    {
                        case '0': // Reload INI file
                            cwmWorldState->ServerData()->Load();
                            break;
                        case '1': // Reload accounts
                            Accounts->Load();
                            break;
...
void checkPC(CSocket *mSock, CChar& mChar)
{
    ...
    else if (mSock->GetTimer(tPC_TRACKING) > (cwmWorldState->GetUICurrentTime() / 10)) // dont send arrow-away packet all the time
    {
...
void checkItem(CMapRegion *toCheck, bool checkItems, UI32 nextDecayItems)
{
    ...
    for (CItem *itemCheck = regItems->First(); !regItems->Finished(); itemCheck = regItems->Next())
    {
        ...
        if (itemCheck->CanBeObjType(OT_BOAT))
        {
            ...
            if (ValidateObject(mBoat) && mBoat->GetMoveType() && (mBoat->GetMoveTime() <= cwmWorldState->GetUICurrentTime() || cwmWorldState->GetOverflow()))
            {
...
void CWorldMain::CheckAutoTimers(void)
{
    ...
    if (accountFlush <= GetUICurrentTime() || GetOverflow())
    {
        ...
        for (MAPUSERNAMEID_ITERATOR I = Accounts->begin(); I != Accounts->end(); ++I)
        {
    ...
    if (nextCheckSpawnRegions <= GetUICurrentTime() && ServerData()->CheckSpawnRegionSpeed() != -1)//Regionspawns
    {
        UI16 itemsSpawned = 0;
        UI16 npcsSpawned  = 0;
        for (auto itr = cwmWorldState->spawnRegions.begin(); itr != cwmWorldState->spawnRegions.end(); ++itr)
            if (itr->second != NULL)
                if (itr->second->GetNextTime() <= GetUICurrentTime())
                    itr->second->doRegionSpawn(itemsSpawned, npcsSpawned);
...
#pragma note("MsgBoardMaintenance() disabled until someone can figure out why it breaks bullein boards!")
    ...
    for (auto itr = cwmWorldState->refreshQueue.begin(); itr != cwmWorldState->refreshQueue.end(); ++itr)
        if (CBaseObject *mObj = itr->first)
...
void checkRegion(CSocket *mSock, CChar& mChar, bool forceUpdateLight)
{
    ...
    else if (forceUpdateLight && mSock != NULL)
        doLight(mSock, cwmWorldState->ServerData()->WorldLightCurrentLevel());
}
