...
UI08 ShiftValue(UI08 toShift, UI08 base, UI08 upper, bool extra)
{
    ...
    else if (toShift >= base && toShift <= upper)
...
void CPITalkRequestUnicode::Receive(void)
{
    ...
    if ((typeUsed&0xC0) == 0xC0)
    {
        ...
        memcpy(unicodeTxt, &(tSock->Buffer()[12]), blockLen - 12); // starts at 12
    ...
    case PARTY_DECLINE:
        {
            ...
            if (leader != NULL)
            {
                ...
                if (leaderSock != NULL) // is it a PC leader?
