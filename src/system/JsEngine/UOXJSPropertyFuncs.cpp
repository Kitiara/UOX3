...
namespace UOX
{
    ...
    JSBool CSpellProps_getProperty(JSContext *cx, JSObject *obj, jsval id, jsval *vp)
    {
        ...
        if (JSVAL_IS_INT(id)) 
        {
            ...
            switch(JSVAL_TO_INT(id))
            {
                ...
                case CSP_MANTRA:
                    tString = JS_NewStringCopyZ(cx, gPriv->Mantra().c_str());
        ...
        if (JSVAL_IS_INT(id)) 
        {
            ...
            switch(JSVAL_TO_INT(id))
            {
                ...
                case CIP_OWNER:        
                    ...
                    break;
                case CIP_VISIBLE:
                    gPriv->SetVisible((VisibleTypes)encaps.toInt());
                    break;
                case CIP_SERIAL:
                    break;
                case CIP_HEALTH:
                    gPriv->SetHP((SI16)encaps.toInt());
                    break;
                case CIP_SCRIPTTRIGGER:
                    gPriv->SetScriptTrigger((UI16)encaps.toInt());
                    break;
                case CIP_WORLDNUMBER:
                    gPriv->SetLocation(gPriv->GetX(), gPriv->GetY(), gPriv->GetZ(), (UI08)encaps.toInt());
                    break;
                case CIP_AMOUNT:
                    gPriv->SetAmount((UI32)encaps.toInt());
                    break;
                case CIP_CONTAINER:
                    if (*vp != JSVAL_NULL)
                    {
                ...
                case CIP_ATT:
                    gPriv->SetLoDamage((SI16)encaps.toInt());
                    gPriv->SetHiDamage((SI16)encaps.toInt());
                    break;
                case CIP_LAYER:
                    gPriv->SetLayer((ItemLayers)encaps.toInt());
                    break;
                case CIP_ITEMSINSIDE:
                    break;
                case CIP_DECAYABLE:
                    gPriv->SetDecayable(encaps.toBool());
                    break;
                case CIP_DECAYTIME:
                ...
                case CCP_TOWN:
                    {
                        ...
                        if (TempTownID == 0xFF)
                            *vp = JSVAL_NULL;
                        else // Should build the town here
                        {
                ...
                case CCP_ATTACKER:
                    {
                        ...
                        if (!ValidateObject(tempChar))
                            *vp = JSVAL_NULL; // Return a JS_NULL
                        else // Otherwise Acquire an object
                        {
                            JSObject *myChar = JSEngine->AcquireObject(IUE_CHAR, tempChar, JSEngine->FindActiveRuntime(JS_GetRuntime(cx)));
                            *vp = OBJECT_TO_JSVAL(myChar);
                    ...
                    break;
                case CCP_RACEGATE:
                    *vp = INT_TO_JSVAL(gPriv->GetRaceGate());
                    break;
                case CCP_SKILLLOCK:
        ...
        if (JSVAL_IS_INT(id)) 
        {
            switch(JSVAL_TO_INT(id))
            {
                ...
                case CCP_ATWAR:
                    ...
                    break;
                case CCP_SPELLCAST:
                    gPriv->SetSpellCast((SI08)encaps.toInt());
                    break;
                case CCP_ISCASTING:
                    {
                ...
                case CCP_ISSHOP:
                    ...
                    break;
                case CCP_ATTACKFIRST:
                    gPriv->SetAttackFirst(encaps.toBool());
                    break;
                case CCP_PARTYLOOTABLE:
                    {
                        Party *toGet = PartyFactory::getSingleton().Get(gPriv);
    ...
    JSBool CSQLMProps_getProperty(JSContext *cx, JSObject *obj, jsval id, jsval *vp)
    {
        if (JSVAL_IS_INT(id)) 
        {
            ...
            switch(realID)
            {
            case CSQLMP_LASTSUCCEEDED: *vp = BOOLEAN_TO_JSVAL(SQLManager::getSingleton().LastSucceeded());
                break;
            default:
                Console.Warning("No such property exists");
                break;
            }
        }
        return JS_TRUE;
