...
void HandleTweakItemButton(CSocket *s, SERIAL button, SERIAL ser, SERIAL type)
{
    ...
    switch(button)
    {
        ...
        case 44: // Spawnobj/Spawnobjlist
            ...
            break;
        default:
            Console << Dictionary->GetEntry(533) << (SI32)button << myendl;
            break;
    }
}
...
void HandleTownstoneButton(CSocket *s, SERIAL button, SERIAL ser, SERIAL type)
{
    ...
    switch(button)
    {
        ...
        case 4: // toggle town title
            ...
            break;
        case 5: // vote for town mayor
            s->target(0, TARGET_VOTEFORMAYOR, 542);
            break;
        case 6: // gold donation
            TextEntryGump( s, ser, static_cast<UI08>(type), static_cast<UI08>(button), 6, 543);
            break;
        case 7: // View Budget
            ourRegion->ViewBudget(s);
            break;
        case 8: // View allied towns
        ...
        case 21: // set taxes
            s->sysmessage(544);
            break;
        case 22: // list town members
            ourRegion->DisplayTownMembers(s);
            break;
        case 23: // force early election
            ourRegion->ForceEarlyElection();
            break;
        case 24: // purchase more guards
            s->sysmessage(545);
            break;
        case 25: // fire a guard
            s->sysmessage(546);
            break;
        case 26: // make a town an ally
            s->target(0, TARGET_TOWNALLY, 547);
            break;
        case 40: // we can't return from mayor menu if we weren't mayor!
            ourRegion->DisplayTownMenu(NULL, s);
            break;
        case 41: // join town!
...
void HandleAccountModButton(CPIGumpMenuSelect *packet)
{
    ...
    for (UI32 i = 0; i < packet->TextCount(); ++i)
    {
        ...
        {
            case 1000:
                username = packet->GetTextUString(i);
                break;
            case 1001:
                password = packet->GetTextUString(i);
                break;
            case 1002:
                emailAddy = packet->GetTextUString(i);
                break;
            default:
...
void HandleHouseButton(CSocket *s, long button, CItem *j)
{
    ...
    switch(button)
    {
        ...
        case 2: // Bestow ownership upon someone else
            s->target(0, TARGET_HOUSEOWNER, 557);
            break;
        case 3: // Turn house into a deed
            deedHouse(s, house);
            break;
        case 4: // Kick someone out of house
            s->target(0, TARGET_HOUSEEJECT, 558);
            break;
        case 5: // Ban somebody
            s->target(0, TARGET_HOUSEBAN, 559);
            break;
        case 6: // Remove someone from ban list
            s->TempInt(1);
            s->target(0, TARGET_HOUSEUNLIST, 560);
            break;
        case 7: // Make someone a friend
            s->target(0, TARGET_HOUSEFRIEND, 561);
            break; 
...
bool CPIGumpMenuSelect::Handle(void)
{
    ...
    {
        case 1: // Tweak Item
            HandleTweakItemButton(tSock, buttonID, id, gumpID);
            break;
        case 2: // Tweak Char
            HandleTweakCharButton(tSock, buttonID, id, gumpID);
            break;
        case 3: // Townstones
        ...
        case 9: // Add Menu
            ...
            break;
        case 11: // Who's Offline
            OffList->ButtonSelect(tSock, static_cast<UI16>(buttonID), static_cast<UI08>(gumpID));
            break;
        case 12: // New Make Menu
            Skills->HandleMakeMenu(tSock, buttonID, static_cast<int>(tSock->AddID()));
            break;
    }
    return true;
}
...
//| Purpose     : Begin GumpDisplay stuff by setting the target, clearing any existing data, and setting the w / h
//o---------------------------------------------------------------------------o
GumpDisplay::GumpDisplay(CSocket *target, UI16 gumpWidth, UI16 gumpHeight) : width(gumpWidth), height(gumpHeight), toSendTo(target)
{
    gumpData.resize(0);
}
