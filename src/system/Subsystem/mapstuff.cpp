...
** -lingo
*/
CMulHandler::CMulHandler() : landTile(0), landTileHS(0), staticTile(0), staticTileHS(0), multiItems(0), multiItemsHS(0), multiIndex(0),
    multiIndexHS(0), multiIndexSize(0), multiSize(0), tileDataSize(0)
{
    LoadMapsDFN();
}
...
UI16 CMulHandler::MultiTile(CItem *i, SI16 x, SI16 y, SI08 oldz)
{
    ...
    if (cwmWorldState->ServerData()->ServerUsingHSMultis())
    {
        ...
        {
            MultiHS_st& multi = SeekIntoMultiHS(multiID, j);
            if ((multi.visible && (i->GetX() + multi.x == x) && (i->GetY() + multi.y == y) && (abs(i->GetZ() + multi.z - oldz) <= 1)))
                return multi.tile;
        }
    }
    else
...
SI08 CMulHandler::MapElevation(SI16 x, SI16 y, UI08 worldNumber)
{
    ...
    if (430 == map.id || 431 == map.id || 432 == map.id || 433 == map.id || 434 == map.id || 475 == map.id ||
        580 == map.id || 610 == map.id || 611 == map.id || 612 == map.id || 613 == map.id)
        return ILLEGAL_Z;
    return map.z;
}
...
bool CMulHandler::DoesStaticBlock(SI16 x, SI16 y, SI08 oldz, UI08 worldNumber, bool checkWater)
{
    ...
    for (Static_st *stat = msi.First(); stat != NULL; stat = msi.Next())
    {
        ...
        if (cwmWorldState->ServerData()->ServerUsingHSTiles())
        {
            ...
            else if (elev >= oldz && stat->zoff <= oldz && (tile.CheckFlag(TF_BLOCKING) && !tile.CheckFlag(TF_WET)))
                return true;
        }
        else
        {
...
bool CMulHandler::DoesDynamicBlock(SI16 x, SI16 y, SI08 z, UI08 worldNumber, bool checkWater, bool waterWalk)
{
    ...
    if (IsValidTile(dt))
        if (cwmWorldState->ServerData()->ServerUsingHSTiles())
        {
            ...
            else if (tile.CheckFlag(TF_BLOCKING) || (checkWater && tile.CheckFlag(TF_WET)) /*|| !tile.CheckFlag(TF_SURFACE) */)
                return true;
        }
        else
        {
