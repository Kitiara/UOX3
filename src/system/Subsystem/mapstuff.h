...
private:
    struct MultiItemsIndex_st
    {
        ...
        SI16 lx, ly, hx, hy;
        SI08 lz, hz;
        MultiItemsIndex_st() : items(NULL), size(-1), lx(SHRT_MAX), ly(SHRT_MAX), hx(SHRT_MIN),
            hy(SHRT_MIN), lz(SCHAR_MAX), hz(SCHAR_MIN) {}
    ...
    struct MultiItemsIndexHS_st
    {
        ...
        SI16 lx, ly, hx, hy;
        SI08 lz, hz;
        MultiItemsIndexHS_st() : items(NULL), size(-1), lx(SHRT_MAX), ly(SHRT_MAX), hx(SHRT_MIN),
            hy(SHRT_MIN), lz(SCHAR_MAX), hz(SCHAR_MIN) {}
        void Include(SI16 x, SI16 y, SI08 z);
    };
