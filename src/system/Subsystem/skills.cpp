...
void cSkills::doStealing(CSocket *s, CChar *mChar, CChar *npc, CItem *item)
{
    ...
    if (objInRange(mChar, npc, DIST_NEXTTILE))
    {
        ...
        if ((!canSteal && RandomNum(1, 5) == 5) || mChar->GetSkill(STEALING) < RandomNum(0, 1001)) //Did they get caught? (If they fail 1 in 5 chance, other wise their skill away from 1000 out of 1000 chance)
        {
            s->sysmessage(882);
            if (npc->IsNpc()) 
                ...
                if ((ttlStats + 1) >= RandomNum(ServStatCap-10, ServStatCap))
                {
                    for (int i = 0; i < 3; i++)
                        if (StatLocks[i] == SKILL_DECREASE)
                            ...
                            if (toDec == 255) 
                                toDec = i;
                            else if (ActualStat[i] > ActualStat[toDec])
                                toDec = i;
