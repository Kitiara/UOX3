...
void CTownRegion::Load(std::vector<UString> dataLines)
{
    ...
    int membercount = 0;
    for (std::vector<UString>::iterator itr = dataLines.begin(); itr != dataLines.end(); ++itr)
    {
        switch (itr-dataLines.begin())
        {
        case 0: // regions.name
            name = itr->empty() ? Dictionary->GetEntry(1117) : itr->substr(0, 49);
            break;
        case 1: // regions.race
            race = itr->toUShort();
            break;
        case 2: // regions.guardowner
            guardowner = itr->empty() ? "" : itr->c_str();
            break;
        case 3: // regions.mayor
            mayorSerial = itr->empty() ? DEFTOWN_MAYOR : itr->toULong();
            break;
        case 4: // regions.priv
            priv = itr->toUByte();
            break;
        case 5: // regions.resourceamount
            goldReserved = itr->toLong();
            break;
        case 6: // regions.taxedid
            taxedResource = itr->empty() ? DEFTOWN_TAXEDRESOURCE : itr->toUShort();
            break;
        case 7: // regions.taxedamount
            taxedAmount = itr->toUShort();
            break;
        case 8: // regions.guardspurchased
            guardsPurchased = itr->toShort();
            break;
        case 9: // regions.timeg
            timeSinceGuardsPaid = itr->toLong();
            break;
        case 10: // regions.timet
            timeSinceTaxedMembers = itr->toLong();
            break;
        case 11: // regions.resourcecollected
            resourceCollected = itr->toLong();
            break;
        case 12: // regions.health
            health = itr->toShort();
            break;
        case 13: // regions.electiontime
            timeToElectionClose = itr->toLong();
            break;
        case 14: // regions.polltime
            timeToNextPoll = itr->toLong();
            break;
        case 15: // regions.world
            break;
        case 16: // regions.numguards
            numGuards = itr->empty() ? DEFTOWN_NUMGUARDS : itr->toUShort();
            break;
        case 17: // regions.member
            if (itr->empty())
            {
                membercount = itr->sectionCount(",");
                for (int i = 0; i < itr->sectionCount(",") + 1; ++i)
                {
                    ...
                    if (!uStr.empty())
                    {
                        ...
                        townMember[location].townMember = uStr.toULong();
                    }
                }
            }
            break;
        case 18: // regions.vote
            if (location != 0xFFFFFFFF && !itr->empty())
            {
                if (itr->sectionCount(",") < membercount)
                    membercount = itr->sectionCount(",");
                ...
                for (int i = 0; i < membercount+1; ++i)
                {
                    UString uStr = itr->section(",", i, i);
                    if (!uStr.empty())
                        townMember[location].targVote = uStr.toULong();
                }
            }
            break;
        case 19: // regions.allytown
            if (itr->empty())
                for (int i = 0; i < itr->sectionCount(",")+1; ++i)
                {
                    UString uStr = itr->section(",", i, i);
                    if (!uStr.empty())
                        alliedTowns.push_back(uStr.toUByte());
                }
            break;
        }
    }
...
UString CTownRegion::Save()
{
    ...
    Str << "('" << static_cast<UI16>(regionNum) << "', ";
    Str << "\"" << name << "\", ";
    Str << "'" << race << "', ";
    Str << "\"" << guardowner << "\", ";
    Str << "'" << mayorSerial << "', ";
    Str << "'" << int((UI08)static_cast<UI16>(priv.to_ulong())) << "', ";
    Str << "'" << goldReserved << "', ";
    Str << "'" << taxedResource << "', ";
    Str << "'" << taxedAmount << "', ";
    Str << "'" << guardsPurchased << "', ";

    if (timeSinceGuardsPaid == 0)
        Str << "'0000-00-00 00:00:00', ";
    else
        Str << "FROM_UNIXTIME(" << timeSinceGuardsPaid << "), ";

    if (timeSinceTaxedMembers == 0)
        Str << "'0000-00-00 00:00:00', ";
    else
        Str << "FROM_UNIXTIME(" << timeSinceTaxedMembers << "), ";

    Str << "'" << resourceCollected << "', ";
    Str << "'" << health << "', ";
    Str << "'" << timeToElectionClose << "', ";
    Str << "'" << timeToNextPoll << "', ";
    Str << "'" << static_cast<UI16>(worldNumber) << "', ";
    Str << "'" << numGuards << "', ";
    ...
    for (int i = 0; i < 2; ++i)
        ...
        {
            Str << "'";
            for (std::vector<townPers>::const_iterator itr = townMember.begin(); itr != townMember.end(); ++itr)
            {
                Str << (i == 0 ? (*itr).townMember : (*itr).targVote);
                if (itr-townMember.begin() != townMember.size()-1)
                    Str << ",";
            }
            Str << "', ";
        }

    if (alliedTowns.empty())
        Str << "NULL";
    else
    {
        Str << "'";
        for (std::vector<UI16>::const_iterator itr = alliedTowns.begin(); itr != alliedTowns.end(); ++itr)
        {
            Str << static_cast<UI16>((*itr));
            if (itr-alliedTowns.begin() != alliedTowns.size()-1)
                Str << ",";
        }
        Str << "'";
    }
...
void CTownRegion::CalcNewMayor(void) // There has got to be a better way than this hideous O(n^2) algy
{
    if (townMember.empty())
        return;
...
CChar * CTownRegion::GetMayor(void) // returns the mayor character
{
    return calcCharObjFromSer(mayorSerial);
}

SERIAL CTownRegion::GetMayorSerial(void) const // returns the mayor's serial
{
    return mayorSerial;
}

bool CTownRegion::AddAsTownMember(CChar& toAdd) // toAdd is the character to add
{
    ...
    for (tag = toScan->First(); !toScan->AtEnd(); tag = toScan->Next())
    {
        ...
        switch((UTag.data()[0]))
        {
            ...
            case 'D':
                ...
                break;
            case 'E':
                if (UTag == "ESCORTS") // Load the region number in the global array of valid escortable regions
                    if (data.toUShort() == 1)
...
void CTownRegion::SendEnemyTowns(CSocket *sock)
{
    ...
    for (auto itr = cwmWorldState->townRegions.begin(); itr != cwmWorldState->townRegions.end(); ++itr)
    {
        CTownRegion *myReg = itr->second;
        if (myReg != NULL)
            if (itr != ourTown && Races->CompareByRace(race, myReg->GetRace()) <= RACE_ENEMY) // if we're racial enemies, and not the same as ourselves
            {
                ++enemyCount;
