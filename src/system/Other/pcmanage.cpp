...
COLOUR validSkinColour(UI16 id, UI16 bodyID)
{
    ...
    case 0x025E: // elven female
        ...
        case 2307:
            rvalue = id;
            break;
        default: // nope, it's not, let's default
            rvalue = 191;
            break;
        }
        break;
...
void CPICreateCharacter::SetNewCharSkillsStats(CChar *mChar)
{
    ...
    for (i = 0; i < ALLSKILLS; ++i)
    {
        ...
        else if (i == fourthSkill && cwmWorldState->ServerData()->ExtendedStartingSkills())
...
void CPICreateCharacter::SetNewCharGender(CChar *mChar)
{
    ...
    else
    {
        ...
        case 1: // Human female
            pGenderID = 0x0191;
            break;
        case 2: // Elf male (human if server doesn't allow elf but client tried anyway)
            pGenderID = (elfCreation ? 0x025D : 0x0190);
            break;
        case 3: // Elf female (human if server doesn't allow elf but client tried anyway)
            pGenderID = (elfCreation ? 0x025E : 0x0191);
            break;
        default:
            break;
...
void HandleDeath(CChar *mChar)
{
    ...
    if (!mChar->IsNpc())
    {
        ...
        {
        case 0x0190: // Human male
            mChar->SetID(0x0192);
            break;
        default:
        case 0x0191: // Human female, or others
            mChar->SetID(0x0193);
            break;
        case 0x025D: // Elf male
            mChar->SetID(0x025F);
            break;
        case 0x025E: // Elf female
            mChar->SetID(0x0260);
            break;
        case 0x029A: // Gargoyle male
            mChar->SetID(0x02B6);
        ...
        case 0x029B: // Gargoyle female
            mChar->SetID(0x02B7);
            break;
        case 0x00B7: // Savage male
        case 0x00B9:
        case 0x02EE:
            mChar->SetID(0x0192);
            break;
        case 0x00B8: // Savage female
        case 0x00BA:
        case 0x02EF:
            mChar->SetID(0x0192);
            break;
