...
CItem *autoStack(CSocket *mSock, CItem *iToStack, CItem *iPack)
{
    ...
    if (iToStack->isPileable())
    {
        ...
        for (CItem *stack = ipCont->First(); !ipCont->Finished(); stack = ipCont->Next())
        {
            ...
            if (stack->isPileable() && stack->GetAmount() < MAX_STACK &&  stack->GetSerial() != itSer && stack->GetID() == itID && stack->GetColour() == itCol)
                // Autostack
...
bool CPIEquipItem::Handle(void)
{
    ...
    {
        CTileHS& tile = Map->SeekTileHS(i->GetID());
        if (!ourChar->AllMove() && (i->GetMovable() == 2 || (i->IsLockedDown() && i->GetOwnerObj() != ourChar) || (tile.Weight() == 255 && i->GetMovable() != 1)))
        {
            Bounce(tSock, i);
            return true;
        }
...
bool DropOnPC(CSocket *mSock, CChar *mChar, CChar *targPlayer, CItem *i)
{
    ...
    else if (isOnline((*targPlayer))) // Trade stuff
    {
...
void getFameTitle(CChar *p, std::string& FameTitle)
{
    if (ValidateObject(p))
    {
        ...
        else if (p->GetKills() > cwmWorldState->ServerData()->RepMaxKills())
...
void handleCharDoubleClick(CSocket *mSock, SERIAL serial, bool keyboard)
{
    ...
    else if (mChar->GetSerial() == serial) // Double-clicked yourself
    {
...
ItemTypes findItemType(CItem *i)
{
    ...
    switch(iType)
    {
        case IT_NOTYPE:
        case IT_COUNT:
            iType = FindItemTypeFromID(i->GetID());
            break;
        default:
            break;
    }
...
bool ItemIsUsable(CSocket *tSock, CChar *ourChar, CItem *iUsed, ItemTypes iType)
{
    ...
    if (ourChar->GetCommandLevel() < CL_CNS)
    {
        ...
        if (iUsed->GetCont() != NULL)
        {
            ...
            if (ValidateObject(iChar) && iChar != ourChar && !(iUsed->IsContType() && cwmWorldState->ServerData()->RogueStatus()))
            {
