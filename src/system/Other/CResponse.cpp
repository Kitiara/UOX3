...
bool WhichResponse(CSocket *mSock, CChar *mChar, std::string text)
{
    ...
    for (UI16 trigWord = mSock->FirstTrigWord(); !mSock->FinishedTrigWords(); trigWord = mSock->NextTrigWord())
    {
        ...
        case TW_KILLS:
            ...
            break;
        case TW_BANK:
            tResp = new CBankResponse();
            break;
        case TW_BALANCE:
            tResp = new CBankResponse(true);
            break;
        case TW_QUESTTAKE:
        ...
        case TW_QUESTDEST:
            ...
            break;
        case TW_TRAIN:
            tResp = new CTrainingResponse(text);
            break;
        case TW_FOLLOW:
        case TW_FOLLOW2:
            tResp = new CPetMultiResponse(text, false, TARGET_FOLLOW, 1310);
            break;
        case TW_COME:
        case TW_FOLLOWME:
            tResp = new CPetComeResponse(false, text);
            break;
        case TW_ALLCOME:
        case TW_ALLFOLLOW:
        case TW_ALLFOLLOWME:
            tResp = new CPetComeResponse(true, text);
            break;
        case TW_KILL:
        case TW_ATTACK:
            tResp = new CPetAttackResponse(false, text);
            break;
        case TW_ALLKILL:
        case TW_ALLATTACK:
            tResp = new CPetAttackResponse(true, text);
            break;
        case TW_FETCH:
        case TW_GET:
        case TW_BRING:
            tResp = new CPetMultiResponse(text, false, TARGET_GUARD, 1316);
            break;
        case TW_FRIEND:
            tResp = new CPetMultiResponse(text, true, TARGET_FRIEND, 1620);
            break;
        case TW_GUARD:
            tResp = new CPetMultiResponse(text, true, TARGET_GUARD, 1104);
            break;
        case TW_ALLGUARD:
        case TW_ALLGUARDME:
            tResp = new CPetGuardResponse(true, text);
            break;
        case TW_STOP:
        case TW_STAY:
            tResp = new CPetStayResponse(false, text);
            break;
        case TW_ALLSTOP:
        case TW_ALLSTAY:
            tResp = new CPetStayResponse(true, text);
            break;
        case TW_TRANSFER:
            tResp = new CPetMultiResponse(text, true, TARGET_TRANSFER, 1323);
            break;
        case TW_RELEASE:
            tResp = new CPetReleaseResponse(text);
            break;
        case TW_VENDORBUY:
            tResp = new CVendorBuyResponse(true, text);
            break;
        case TW_BUY:
            tResp = new CVendorBuyResponse(false, text);
            break;
        case TW_VENDORSELL:
            tResp = new CVendorSellResponse(true, text);
            break;
        case TW_SELL:
            tResp = new CVendorSellResponse(false, text);
            break;
        case TW_VENDORVIEW:
            tResp = new CVendorViewResponse(true, text);
            break;
        case TW_VIEW:
            tResp = new CVendorViewResponse(false, text);
            break;
        case TW_VENDORGOLD:
            tResp = new CVendorGoldResponse(true, text);
            break;
        case TW_COLLECT:
        case TW_GOLD:
            tResp = new CVendorGoldResponse(false, text);
            break;
        case TW_VENDORSTATUS:
            tResp = new CVendorStatusResponse(true, text);
            break;
        case TW_STATUS:
            tResp = new CVendorStatusResponse(false, text);
            break;
        case TW_VENDORDISMISS:
            tResp = new CVendorDismissResponse(true, text);
            break;
        case TW_DISMISS:
            tResp = new CVendorDismissResponse(false, text);
            break;
        case TW_HOUSEBAN:
        ...
        case TW_BOATFORWARD:
        case TW_BOATUNFURL:
            tResp = new CBoatMultiResponse(1);
            break;
        case TW_BOATBACKWARD:
            tResp = new CBoatMultiResponse(2);
            break;
        case TW_BOATSTOP:
        case TW_STOP2:
        case TW_BOATFURL:
            tResp = new CBoatMultiResponse(0);
            break;
